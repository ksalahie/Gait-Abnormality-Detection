...
- **Parkinson's Disease:** Exhibited shorter stride times (0.85s) and higher stride time coefficient of variation (2.7%) compared to healthy controls (1.3%).
- **Cerebral Palsy:** Demonstrated highest double support time (28.1%) and lowest gait speed (0.70 m/s).
...
Processing raw, continuous stride intervals via a CNN-LSTM architecture proved vastly superior to extracting static averages. By capturing the fractal dynamics and sequential deterioration over 60-stride windows, the deep learning approach achieved significant performance gains over the baseline Random Forest ensemble:
